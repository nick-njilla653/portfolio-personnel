...
const CareerCard = ({
  ...
  const yearText = `an${durationYears > 1 ? "s" : ""}`;
  ...
  if (durationMonths > 0 || durationYears === 0) {
    durationText += `${durationMonths} mois`;
  }

  const hideText = "Masquer";
  const showText = "Afficher";
  const responsibilityText = "responsabilités";
        ...
        <div className="space-y-2 text-sm text-neutral-600 dark:text-neutral-400">
          ...
          <div className="flex flex-col gap-2 text-[13px] md:flex-row">
            <div className="flex gap-1 text-neutral-600 dark:text-neutral-400">
              ...
              <span>{end_date ? format(endDate, "MMM yyyy") : "Présent"}</span>
            </div>
