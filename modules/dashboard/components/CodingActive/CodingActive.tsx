...
const CodingActive = () => {
    ...
    const formatLastUpdate = (): void => {
      ...
      if (lastUpdateDate) {
        const zoneDate = utcToZonedTime(
          zonedTimeToUtc(lastUpdateDate, "Africa/Douala"),
          "Africa/Douala",
        );
  ...
  return (
    <section className="space-y-2">
      <SectionHeading
        title="Activité de Codage"
        icon={<WakatimeIcon className="mr-1" />}
      />
      <SectionSubHeading>
        Mes statistiques de temps de codage et langages utilisés
        <Link
          href="https://wakatime.com/@njilla"
          target="_blank"
          className="text-sm text-neutral-600 hover:text-neutral-800 dark:text-neutral-500 dark:hover:text-neutral-400"
        >
          Dernière mise à jour : {renderLastUpdate()}
        </Link>
      </SectionSubHeading>
